...
def count_expert_num_tokens(
    ...
    # SHIZUHA / vLLM PR #48734 (issue #48650): pin BLOCK_SIZE to a fixed 1024
    # instead of next_power_of_2(min(numel, 1024)). The runtime-token-count
    # -varying tl.constexpr caused a lazy Triton JIT on the FIRST real
    # completion (an uncovered token-count bucket the [1,2] warmup never hit).
    # In TP=2 that JIT fired on one rank while the peer was blocked at the next
    # collective -> ranks desynced -> EngineCore shm_broadcast/NCCL deadlock
    # (Ready /health but completions hang forever). 1024 >= every value it
    # replaced and the masked loop (cdiv(numel, BLOCK_SIZE) with
    # offsets < numel - x*BLOCK_SIZE) is correct for all numel, so the kernel
    # now compiles exactly once at warmup and never JITs on the serving path.
    BLOCK_SIZE = 1024
